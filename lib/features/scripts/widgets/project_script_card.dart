...
import '../screens/script_assets_screen.dart';
import '../../../core/theme/app_theme.dart';

class ProjectScriptCard extends StatelessWidget {
  final ProjectScript script;
  final VoidCallback? onTap;

  const ProjectScriptCard({super.key, required this.script, this.onTap});
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Row(
                    children: [
                      Icon(Icons.article, color: Colors.blue[400], size: 24),
              ...
              // Detalles de tokens (prompt y completion)
              if (script.promptTokens > 0 || script.completionTokens > 0) ...[
                Row(
                  children: [
                    Text(
                      'Prompt: ${script.promptTokens}',
                      style: TextStyle(fontSize: 11, color: Colors.grey[500]),
                    ),
                    const SizedBox(width: 16),
                    Text(
                      'Completion: ${script.completionTokens}',
                      style: TextStyle(fontSize: 11, color: Colors.grey[500]),
                    ),
                  ],
                ),
              ...
              Row(
                children: [
                  ...
                  if (!script.hasMixedAudio && !script.hasMixedMedia) ...[
                    Icon(Icons.pending, size: 16, color: Colors.grey[500]),
                    const SizedBox(width: 4),
                    Text(
                      'Sin medios',
                  ...
                  GestureDetector(
                    onTap: () {
                      Navigator.of(context).push(
                        MaterialPageRoute(
                          builder:
                              (context) => ScriptAssetsScreen(
                                scriptId: script.id,
                                scriptTitle:
                                    'Script ${script.id.substring(0, 8)}',
                              ),
                        ),
                      );
                    },
